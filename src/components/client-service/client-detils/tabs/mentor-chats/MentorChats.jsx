...
import backgroundImage from "src/assets/images/bg-whats-app.jpg";
import LayoutContainerTitle from "src/components/client-service/layout/LayoutContainerTitle";

const MentorChats = ({ theme }) => {
  return (
    <>
      <LayoutContainerTitle  title={"Mentor Chat"} />
      <Box mt={2} height={400} component="div" sx={{ position: "relative" }}>
        <img
          src={backgroundImage}
          alt="WhatsApp Background"
          className="bg-whats-app"
          style={{
            background:
              theme.palette.mode === "dark"
                ? "rgba(226, 219, 211,0.1)"
                : "rgba(226, 219, 211,0.4)",
          }}
        />
        <Stack
          direction={"column"}
            ...
            <Box
              sx={{
                ...
                "&:before": {
                  ...
                  borderTopRight: "20px solid transparent",
                  borderTop: ` 15px solid ${'background.default'}`,
                },
              }}
              key={item.id}
            >
                  {item.message}
                  <Typography color="secondary" sx={{fontSize:12}} mt={2}>06:45 PM</Typography>
            </Box>
          ))}
        </Stack>
      </Box>
...
const mentorChatsData = [
  {
    senderId: 212121,
    receiverId: 122121,
    message:
      "Lorem ipsum dolor sit amet consectetur, adipisicing elit. A veritatis sed, exercitationem quibusdam cupiditate laboriosam eligendi nesciunt autem ducimus obcaecati!",
    attachment: [
